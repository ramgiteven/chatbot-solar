...
def get_coordinates_from_geocoding_api(address):
...
def get_solar_data_from_solar_api(lat, lng):
...
def extract_financial_analyses_list_from_solar_data(solar_data):
  try:
...
def get_financial_data_for_address(address):
  lat, lng = get_coordinates_from_geocoding_api(address)
  if not lat or not lng:
    return {"error": "Could not get coordinates for the address provided."}
  return extract_financial_analyses_list_from_solar_data(get_solar_data_from_solar_api(lat, lng))


def find_closest_financial_analysis_with_financial_data(user_bill, financial_analyses):
  closest_match = None
...
def simplify_financial_data_with_gpt(data):
  try:
...
def solar_panel_calculations_output(address, monthly_bill):
  print(
  ...
  closest_financial_analysis = find_closest_financial_analysis_with_financial_data(
      int(monthly_bill), financial_analyses)
  if closest_financial_analysis:
    return simplify_financial_data_with_gpt(closest_financial_analysis)
  else:
